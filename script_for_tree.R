...
p2 <- ggtree(n) + 
  geom_tiplab(linesize=0.5,size=3.2,offset=0.05,family='Cambria')+  
  #label branch tips and here we can change font(family,you need extrafont package for this step), size and colour
  geom_hilight(node=68, fill="green1", alpha=0.2,extend=0.95)+      
  #that's how we can highlight different clade
  ggplot2::xlim(0, 4.5)+ggplot2::ylim(0, 60)+                       
  #sometimes you need set your own scale limits if your tree doesn't fit in defolt limits
  geom_hilight(node = 61,fill="red",alpha = 0.2,extend=1.4)+        
  #now let's highlight a little bit more clades
  ...
  geom_cladelabel(node=61, label="Mitochondrial", align=TRUE,  offset = 1, color='red',fontsize = 4,barsize = 1.2)+  
  #you also can add text label for your clade
  ...
  geom_nodelab(hjust=1,vjust=.3, size=5,color='darkred')   #for label nodes chosen previously add darkred circles
...
d4 = data.frame(id=rep(n$tip.label, each=22), val=r$V8)
#vector r$V8 contains values which match tip labels and for each tips there are EXACTLY 22 values  in vector r$V8 and these values are in the same order as tips in n$tip.label 
